...
class gap_reg_weighted(torch.nn.Module):
    def __init__(self, mode = "dp"):
        super(gap_reg_weighted, self).__init__()
        self.mode = mode

    def forward(self, y_pred, s, y_gt, weight):
        if self.mode == "eo":
            y_pred = y_pred[ y_gt == 1 ]
            s = s[ y_gt == 1 ]
        y0 = y_pred[s == 0]
        ...
        y0_mean = torch.sum(y0@weight0) / torch.sum(weight0)
        y1_mean = torch.sum(y1@weight1) / torch.sum(weight1)

        reg_loss = torch.abs(y0_mean - y1_mean )
        return reg_loss, torch.Tensor([0]), torch.Tensor([0]), torch.Tensor([0])
...
class PRLoss(torch.nn.Module):#using linear
    ...
    def forward(self, y_pred, s, y_gt):
        ...
        Dxisi = torch.stack((N_male,N_female),axis=0) #male sample, #female sample
        # Pr[y|s]
