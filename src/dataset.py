...
def load_census_income_kdd_data(path="/ data/han/data/fairness/census-income-mld/", sensitive_attributes="sex"):
    data1 = pd.read_csv(
        os.path.join(path, "census-income.data"),
        header=None,
    ...
    data2 = pd.read_csv(
        os.path.join(path, "census-income.test"),
        header=None,
        names=[
            "age",
            "workclass",
    ...
    X = input_data.drop(columns=["class", sensitive_attributes]).fillna("Unknown").pipe(pd.get_dummies, drop_first=False)
    return X, y, s
...
def load_acs_dataset(task=None, sensitive_attributes="sex", survey_year="2018", horizon="1-Year", states=["CA"]):
    ...
    data_source = ACSDataSource(survey_year=survey_year, horizon=horizon, survey="person", root_dir="/data/han/data/fairness/folktables/")
...
def load_folktables_employment_5year(task=None, sensitive_attributes="sex"):
    ...
    data_source = ACSDataSource(survey_year="2018", horizon="5-Year", survey="person", root_dir="/data/han/data/fairness/folktables/")
